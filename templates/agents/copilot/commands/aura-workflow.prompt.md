---
name: aura-workflow
description: "Aura-SDD: Run, resume, list, and manage workflow automation pipelines."
mode: agent
---
...
**Built-in workflow: full-sdd** — runs the complete pipeline:
```
...
**Writing custom workflows** — definitions live in `.aura/workflows/definitions/` as JSON files:
```json
{
  "name": "my-workflow",
...
**State persistence** — workflow runs saved to `.aura/workflows/runs/<run-id>/state.json`. Resume from last completed step after any interruption:
```bash
