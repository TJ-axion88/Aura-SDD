...
3. **Assess CONFIDENCE**: `HIGH` (certain) | `MEDIUM` (likely, minor uncertainty) | `LOW` (speculative)

4. **Determine NEXT_ACTION**: `RETRY_TASK` (within boundary, HIGH/MEDIUM) | `BLOCK_TASK` (outside boundary or LOW) | `STOP_FOR_HUMAN` (SPEC_CONFLICT / TASK_SCOPE_EXCEEDED / CONSTITUTIONAL_VIOLATION)
...
**Completion criteria:** ROOT_CAUSE_CATEGORY assigned (one of 10), CONFIDENCE with rationale, root cause identified (not symptom), fix plan specific for new implementer, NEXT_ACTION determined.
