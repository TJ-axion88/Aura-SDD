---
description: "Aura-SDD: Root-cause-first debugging when /aura-review rejects twice. Produces a fix plan for the next Implementer. Called by /aura-impl."
---
...
2. **Trace root cause** — do not assume the obvious failure point. Trace backward:
   ...
   Use web search if the error involves external library behavior, platform-specific behavior, or a known framework bug.
...
**Completion criteria:** Root cause identified (not just symptom), fix plan specific enough for a new implementer with no prior context, verification steps listed.
